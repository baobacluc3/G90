...
router.get("/appointments", (req, res) => {
  ...
  let query = `
    SELECT lh.*, tk.HoTen as userName 
  ...
  if (search) {
    query += " AND (lh.HoTen LIKE ? OR tk.HoTen LIKE ?)";
    countQuery += " AND (lh.HoTen LIKE ? OR tk.HoTen LIKE ?)";
...
router.post("/appointments", (req, res) => {
  ...
  const query = `
    INSERT INTO LichSuDatLich (HoTen, SoDienThoai, DichVu, ChiNhanh, TenThuCung, Ngay, Gio, GhiChu, ID_TaiKhoan, TrangThai) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'Chờ xác nhận')
  `;
